  ...
  <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div
      ...
      <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
        <h2 class="font-semibold text-slate-800 dark:text-slate-100 text-2xl tracking-tight">Anunciantes </h2>
      </header>
      <div class="p-3">
          ...
          <table id="tabladatos" class="display text-lg" style="width:100%">
            ...
            <tbody>
              ...
              @foreach ($inmuebles as $item)
                <tr>
                  <td>{{ $item->producto }}</td>
                  <td>{{ $item->district_description }}</td>
                  ...
                  <td>
                    <form method="POST" action="">
                      ...
                      <input type="checkbox" id="hs-basic-usage"
                        ...
                        id='{{ 'v_' . $item->id }}' data-field='status' data-idService='{{ $item->id }}'
...
<script>
  $('document').ready(function() {
    ...
    $(".btn_swithc").on("change", function() {
      ...
      $.ajax({
        url: "{{ route('anuncia.updateVisible') }}",
        method: 'POST',
        data: {
    ...
    /*  $(".btn_delete").on("click", function(e) {
       ...
       Swal.fire({
         ...
         text: "Vas a eliminar un anunciante",
         icon: "warning",
         ...
         if (result.isConfirmed) {
           ...
           $.ajax({
             ...
             url: "{{ url('anuncia.borrar') }}", //cambiar a route
             method: 'POST',
             data: {
               _token: $('input[name="_token"]').val(),
               id: id,
