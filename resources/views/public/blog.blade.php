        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-12">
            <div class="md:basis-5/6 order-2 md:order-1 flex flex-col gap-10">
                <div class="flex flex-col gap-5" data-aos="fade-up" data-aos-duration="150">
                    ...
                    <div>
                        <a href="#" class="text-[#2E609D] font-roboto font-normal text-text16 flex justify-start items-center gap-3"> <span>Leer más</span> 
                            <div>
                    ...
                    @foreach ($blogs as $item)
                        ...
                        <div class="flex flex-col gap-5" data-aos="fade-up" data-aos-duration="150">
                            <div class="flex justify-center items-center">
                                ...
                                <img src="{{asset($item->url_image)}}" alt="blog" class="w-full h-[300px] object-cover rounded-xl">
                            </div>
                            <div class="flex justify-start items-center gap-5">
                                <p class="text-white font-roboto font-semibold text-text14 bg-[#2E609D] py-2 px-4 rounded-lg">{{-- Categoría --}} {{$item->categories->name}}</p>
                                <p class="text-[#2E609D] font-satoshiBold text-text14">Publicado 5 min</p>
                            </div>
                            <div class="flex flex-col gap-1">
                                <h2 class="text-[#000929] font-satoshiBold text-text24 leading-tight">{{-- Nunc faucibus, augue at bibendum --}}
                                    {{$item->title}}
                                </h2>
                                <p class="text-[#000929] text-opacity-50 text-text16 font-satoshiRegular">
                                    {{-- Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros. --}}
                            ...
                            <div>
                                <a href="{{route('post', $item->id)}}" class="text-[#2E609D] font-roboto font-normal text-text16 flex justify-start items-center gap-3"> <span>Leer más</span> 
                                    <div>
                                        <img src="{{asset('images/svg/imagen_43.svg')}}" alt="arrow">
                                    </div> 
                                </a>
            ...
            <div class="md:basis-1/6 order-1 md:order-2 flex flex-col gap-5" data-aos="fade-up" data-aos-duration="150">
                ...
                @php
                    ...
                    $filtro = end($parts);  
                @endphp
                <a href="/blog/0" class="text-[#000929]  text-text18 py-3 px-4 rounded-lg {{$filtro == 0 ? 'text-opacity-100 font-satoshiBold text-white bg-[#2E609D]' : 'font-satoshiRegular text-opacity-50'}}">Todas</a>
                
                <div class="flex flex-col gap-3">
                    @foreach ($categorias as $item)
                        @if ($item->blogs->where('visible', 1)->count() != 0)
                            <a href="/blog/{{$item->id}}" class="text-[#000929] text-text18 py-3 px-4 rounded-lg
