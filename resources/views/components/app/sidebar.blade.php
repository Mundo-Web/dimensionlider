    ...
    <div class="space-y-8">
      ...
      <div>
        <h3 class="text-xs uppercase text-slate-500 font-semibold pl-3">
          ...
          <span class="lg:hidden lg:sidebar-expanded:block 2xl:block">Dimensión Lider - Backend</span>
        </h3>
        <ul class="mt-3">
          ...
          <!-- Blog -->
          <li
            class="px-3 py-2 rounded-sm mb-0.5 last:mb-0 @if (in_array(Request::segment(2), ['categoriasBlog'])) {{ 'bg-slate-900' }} @endif">
            <a class="block text-slate-200 hover:text-white truncate transition duration-150 @if (in_array(Request::segment(2), ['categoriasBlog'])) {{ 'hover:text-slate-200' }} @endif"
              href="{{ route('categoriasBlog.index') }}">
              <div class="flex items-center">
                <svg class="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                  <path
                    class="fill-current @if (in_array(Request::segment(2), ['categoriasBlog'])) {{ 'text-indigo-500' }}@else{{ 'text-slate-600' }} @endif"
                    d="M1 3h22v20H1z" />
                  <path
                    class="fill-current @if (in_array(Request::segment(2), ['categoriasBlog'])) {{ 'text-indigo-300' }}@else{{ 'text-slate-400' }} @endif"
                    d="M21 3h2v4H1V3h2V1h4v2h10V1h4v2Z" />
                </svg>
                <span
